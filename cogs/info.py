...
class info(commands.Cog):
    ...
    async def help(
      ...
      embed.add_field(name = '<:king:1005355877278154814>  Админские Штучки', value = f'`{prefix}art` `{prefix}artmany` `{prefix}poll` `s.role` `s.ticket` `{prefix}proof` `{prefix}recruit` `{prefix}evmanage` `{prefix}topmanage`', inline=False)
